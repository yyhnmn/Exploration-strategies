...
from agents.NoisyNet import NoisyNet

class NoisyNetAgent(BaseAgent):
    def __init__(self, features, actions, params):
        super().__init__(features, actions, params)
        self.policy_net = NoisyNet(features, self.h1, self.h2, actions).to(device)
        self.target_net = NoisyNet(features, self.h1, self.h2, actions).to(device)
